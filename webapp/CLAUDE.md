...
- ウィジェットの key に値を書き込んで「自動リセット」しない。ウィジェット状態と競合し、以後の操作が反映されなくなるため。無効化は使用時に AND を取る
- 選択肢や初期値が動的に変わるウィジェットには必ず key を付ける。key 無しで value= を変えると別ウィジェット扱いになり、ユーザーの選択が失われるため
